          ...
          <Card key={log.id} className="p-4 hover:shadow-md transition-shadow">
            ...
            {log.image && (
              <div className="mt-4 overflow-hidden rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm max-w-2xl bg-slate-50 dark:bg-slate-900">
                <img src={log.image} alt="Evidencia fotográfica" className="w-full max-h-80 object-cover hover:scale-[1.01] transition-transform duration-500 cursor-pointer" onClick={() => window.open(log.image, '_blank')} />
                <div className="p-2 bg-white dark:bg-slate-800 border-t dark:border-slate-700 text-[10px] text-slate-400 flex items-center gap-1"><LinkIcon size={10} /> Link externo</div>
              </div>
            )}
          </Card>
        ))}
