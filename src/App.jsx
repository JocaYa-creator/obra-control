                 ...
                 {(data.tasks || []).map(task => (
                   <div key={task.id} className="flex flex-col gap-1 text-sm border-b border-slate-100 dark:border-slate-700 pb-2 last:border-0 print:border-slate-300">
                     <div className="flex justify-between items-center">
                       <span className={`${task.completed ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-300 print:text-black'}`}>
                         {task.task}
                         {task.deadline && (
                           <span className="ml-2 text-[10px] font-normal text-slate-400 dark:text-slate-500 print:text-slate-500 italic">
                             — Vence: {task.deadline === 'Sugerido por IA' ? 'IA' : task.deadline}
                           </span>
                         )}
                       </span>
